              ...
              <table class="table table-advance" id="datatable">
                  <thead>
                      <tr>
                          ...
                          <th>{{trans('admin/lmscourse.username')}}</th>
                          <th>{{trans('admin/lmscourse.fullname')}}</th>
                          <th>{{trans('admin/lmscourse.email_id')}}</th> 
                          <th>{{trans('admin/lmscourse.created_on')}}</th>
...
<script type="text/javascript">
 ...
 function updateCheckBoxVals(){
    ...
    $(document).ready(function(){
      ...
      "serverSide": true,
      "ajax": {
              "url": "{{URL::to('/cp/lmscoursemanagement/user-list-ajax')}}",
              "data": function ( d ) {
                  d.filter = $('[name="filter"]').val();
                  d.view = "iframe";
                  d.relfilter = $('[name="relfilter"]').val();
				  
  <?php if(isset($cid) && preg_match('/^\d+$/',$cid)) echo 'd.cid = "'.$cid.'"' ?>;
  <?php if(isset($bid) && preg_match('/^\d+$/',$bid)) echo 'd.bid = "'.$bid.'"' ?>;
<?php if(isset($pid) && preg_match('/^\d+$/',$pid)) echo 'd.pid = "'.$pid.'"' ?>;
