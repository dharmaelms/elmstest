            ...
            <div class="box">
                 <div class="box-title">
                    ...
                    <div class="btn-toolbar pull-right" > 
                        <a href="{{URL::to('cp/reports/individual-channel-performance/')}}/{{$channel_id}}" class="show-tooltip btn btn-primary fa fa-angle-left"> <?php echo trans('admin/reports.back'); ?></a>
                    </div>
                ...
                <div class="box-content" style="overflow-x:scroll"> 
                    ...
                    <table class="table table-advance" id="datatable">
                        <thead>
                            <tr>
                                <th style="text-align: left">{{ trans('admin/reports.user_name') }} </th>
                                <th style="text-align: left"> {{trans('admin/reports.score_wp')}} ({{trans('admin/reports.out_of')}} {{$total_mark}})</th>
                                <th style="text-align: left">{{ trans('admin/reports.score') }}</th>
                                <th style="text-align: left" title="{{$quiz_max_time}}"> {{ trans('admin/reports.time_taken') }}</th>
                                @if(isset($ques_list))
                                    @foreach($ques_list as $key => $ques)
                                        <th style="text-align: left" title="{{strip_tags($ques)}}" >Q {{$key+1}}</th>
                                    @endforeach
                                @endif
                            </tr>
                        </thead>
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                window.datatableOBJ = $('#datatable').on('processing.dt',function(event,settings,flag){
                    ...
                    "serverSide": true,
                    "ajax": {
                        "url": "{{URL::to('cp/reports/ajax-quiz-performance-by-question/')}}",
                        "type" : "POST",
                        "data": function ( d ) {
