                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="alert alert-info" id = "info_report" style="display:none">
                                ...
                                <p>{{trans('admin/reports.user_channel_compl_disc')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <table class="table fill-head table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>{{$user_name}}'s {{trans('admin/reports.compl')}}</th>
...
<script>
    ...
    function renderChart (data, xaxis, id_channel_var, avg_data, user_name) {
        $('#container').highcharts({
                    ...
                    cursor: 'pointer',
                        point: {
                            events: {
                                click: function(e) {
                                    var url = '{{URL::to('/')}}';
                                      location.href = url+'/cp/reports/individual-channel-user-completion/'+id_channel_var[this.index]+'/'+user_id_js
                                    }
                                    }
                                },  
                            },
            ...
            series : [{
                name : user_name +"'s {{trans('admin/reports.compl')}}",
                data : data,
            },{
    ...
    $("#selected_feed").change(function () {
        ...
        location.href = url+'/cp/reports/individual-channel-user-completion/'+FeedSelected+'/'+user_id_js;
    });
    $('#exp_report').click(function(){
        var url = '{{URL::to('/')}}';
        location.href = url+'/cp/reports/csv-user-completion-report/'+user_id_js;
    });
    ...
    function ajaxCallFunc(){
        ...
        $.ajax({
            type:'GET',
            url : url+'/cp/reports/ajax-user-channel-completion/'+no_set+'/'+user_id_js
        })
        .done(function(response){
            var data = response.data;
            var data_avg = response.avg_data;
            var xaxis = response.xaxis;
            var id_channel =response.id;
            var html_temp = "";
            var url_com = url+'/cp/reports/individual-channel-user-completion';
            if(data.length > 0){
                $('#no_record').hide();
                if($.isArray(data)){
                    $.each(data, function(key, ele){
                        html_temp+="<tr><td><a href = '"+url_com+"/"+id_channel[key]+"/"+user_id_js+"/"+xaxis[key]+"'>";
                        html_temp+=xaxis[key]+"</a></td><td>"+ele+"</td><td>"+data_avg[key]+"</td></tr>";
                    });
                }
                if(html_temp != ""){
                    $('#data_tbl_id').html(html_temp);
                }
                renderChart(data, xaxis, id_channel, data_avg, user_name);
            }else{
                $('#no_record').show();
                current--;
            }
