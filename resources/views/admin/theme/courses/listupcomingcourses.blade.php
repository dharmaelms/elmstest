	    ...
	    <div class="col-md-12">
	        <div class="box">
	            ...
	            <div class="box-content">
	            	@if($UpcomingCourses['configuration'] == 'manual')
	            		@if(has_admin_permission(ModuleEnum::HOME_PAGE, HomePagePermission::ADD_UPCOMING_COURSES))
		                    <div class="row">
				            	<div class="col-md-12">
					            	<div class="btn-toolbar pull-right clearfix">
					            		<a href="{{URL::to('/cp/upcomingcourses/course-iframe?from=upcoming')}}" class='productrel btn btn-primary btn-sm' data-info="upcoming" data-text="Add to list"><i class="fa fa-plus"></i> {{trans('admin/courses.add_to_list')}}</a>
					            	</div>
				            	</div>
			            	</div>
			            @endif
                    ...
                    <table class="table table-advance" id="datatable">
				        <thead>
				            <tr>
				                <th>{{trans('admin/courses.name')}}</th>
		...
		<script>
	    	...
	    	$(document).ready(function(){
	    		...
	    		window.datatableOBJ = $datatable.on('processing.dt',function(event,settings,flag){
	    			...
	    			"serverSide": true,
					"ajax": {
			            "url": "{{URL::to('/cp/upcomingcourses/upcoming-courses-ajax/')}}",
			            "data": function ( d ) {
