	        ...
	        <div class="box">
				...
				<div class="col-md-12 margin-bottom-20">
					<div class="btn-toolbar pull-right clearfix">
            		    <div class="btn-group">
							<input type="hidden" id="export_link_ug" name="export_link_ug" value="{{URL::to('/cp/bulkimport/usergroup-pack-export/'.$enrol_level.'/'.$program_sub_type )}}">
							<a class="btn btn-primary btn-sm" id= "export_link_usergroup"  href="{{URL::to('/cp/bulkimport/usergroup-pack-export/'.$enrol_level.'/'.$program_sub_type)}}"><?php echo trans('admin/user.export_log'); ?></a>
							<a class="btn btn-primary btn-sm" style= "margin-left:5px;"
							   title="<?php echo trans('admin/program.import_ug_package_template'); ?>"
							   href="{{URL::to('/cp/bulkimport/import-package-enrol-template/')}}">
								<i class="fa fa-download">&nbsp;</i>
                                <?php echo trans('admin/program.import_ug_package_template'); ?>
							</a>
							<a class="btn btn-circle show-tooltip" title="{{ trans('admin/program.ug_package_import_help') }}"
                    ...
                    <form class="form-horizontal" action="" name="filterform">
			            	...
			            	<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12 margin-bottom-20">
									...
									<div class="form-group last">
										...
										<a href="{{URL::to('/cp/bulkimport/usergroup-package-report')}}"><button type="button" class="btn"><?php echo trans('admin/user.clear') ?></button></a>
										</div>
									</div>
                    ...
                    <table class="table table-advance" id="datatable">
				        <thead>
				            <tr>
							    <th>Usergroup</th>
							    <th>{{Lang::get("admin/program.package")}}</th>
								<th>Short Name</th>
	    ...
	    <div class="modal-dialog">
	                    ...
	                    <div class="col-md-12">
	                        <div class="box">
	                            <div class="box-title">
	                                ...
	                                <h3><i class="icon-file"></i><?php echo trans('admin/program.ug_package_import_information'); ?></h3>
	                            </div>
	                        </div>
	                    </div>
	                </div>
            ...
            <!--content-->
	      		<div class="modal-body">
	                <br>
	                <ul>
	                	<?php echo trans('admin/program.ug_package_import_help_note'); ?>
	                </ul>
	                <br>
			  	</div>
			  	<!--footer-->
...
<!--end of help-->
	    <script>
	    	...
	    	$(document).ready(function(){
	    		...
	    		window.datatableOBJ = $datatable.on('processing.dt',function(event,settings,flag){
	    			...
	    			"serverSide": true,
					"ajax": {
			            "url": "{{URL::to('/cp/bulkimport/package-usergroup-list')}}",
			            "data": function ( d ) {
				...
				$datatable.on('xhr.dt',function(e, settings, json, xhr){
					...
					{
						$("#export_link_usergroup").removeAttr("href").css('cursor', 'default');
					}
		...
		$("#export_link_usergroup").click(function() {
		var selectedValue = $('#filter').find('option:selected').val();
		var created_date = $('#created_date').val();
		
		var exportlink = $('#export_link_ug').val();
		$('#export_link_usergroup').attr('href',(exportlink+"/"+selectedValue+"/"+created_date));
		});
