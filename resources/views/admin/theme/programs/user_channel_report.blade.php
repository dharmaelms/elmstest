	        ...
	        <div class="box">
				...
				<div class="col-md-12 margin-bottom-20">
					<div class="btn-toolbar pull-right clearfix">
            		    <div class="btn-group">
							<input type="hidden" id="export_link" name="export_link" value="{{URL::to('/cp/bulkimport/user-channel-export/'.$enrol_level.'/'.$program_sub_type )}}">
							<a class="btn btn-primary btn-sm" id= "export_link_user"  href="{{URL::to('/cp/bulkimport/user-channel-export/'.$enrol_level.'/'.$program_sub_type)}}"><?php echo trans('admin/user.export_log') ?></a>
							<a class="btn btn-primary btn-sm" style= "margin-left:5px;"
							   title="<?php echo trans('admin/program.import_user_channel_template'); ?>"
							   href="{{URL::to('/cp/bulkimport/import-program-enrol-template/'.$enrol_level.'/'.$program_sub_type)}}">
								<i class="fa fa-download">&nbsp;</i>
                                <?php echo trans('admin/program.import_user_channel_template'); ?>
							</a>
							<a class="btn btn-circle show-tooltip" title="{{ trans('admin/program.user_channel_import_help') }}"
                    ...
                    <form class="form-horizontal" action="" name="filterform">
			            	...
			            	<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12 margin-bottom-20">
									...
									<div class="form-group last">
										...
										<a href="{{URL::to('/cp/bulkimport/user-channel-report')}}"><button type="button" class="btn"><?php echo trans('admin/user.clear') ?></button></a>
										</div>
									</div>
                    ...
                    <table class="table table-advance" id="datatable">
				        <thead>
				            <tr>
							    <th>Username</th>
							    <th>{{Lang::get("admin/program.channel")}}</th>
								<th>Short Name</th>
	    ...
	    <div class="modal-dialog">
	                    ...
	                    <div class="col-md-12">
	                        <div class="box">
	                            <div class="box-title">
	                                ...
	                                <h3><i class="icon-file"></i><?php echo trans('admin/program.user_channel_import_information'); ?></h3>
	                            </div>
	                        </div>
	                    </div>
	                </div>
            ...
            <!--content-->
	      		<div class="modal-body">
	                <br>
	                <ul>
	                	<?php echo trans('admin/program.user_channel_import_help_note'); ?>
	                </ul>
	                <br>
			  	</div>
			  	<!--footer-->
...
<!--end of help-->
	    <script>
	    	...
	    	$(document).ready(function(){
	    		...
	    		window.datatableOBJ = $datatable.on('processing.dt',function(event,settings,flag){
	    			...
	    			"serverSide": true,
					"ajax": {
			            "url": "{{URL::to('/cp/bulkimport/channel-user-list')}}",
			            "data": function ( d ) {
		...
		$('#export_link_user').attr('href',(exportlink+"/"+selectedValue+"/"+created_date));
		});
