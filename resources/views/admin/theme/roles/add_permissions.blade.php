...
<style type="text/css">
  .cs-label {
       padding-top: 14px !important;text-align: center !important;
  }
  .b-left-0 {
...
<div class="row custom-box">
    <div class="col-md-12">
        <div class="box">
            <div class="box-title">
                <h3><i class="icon-file"></i>{{ trans('admin/role.add_permissions') }}</h3>
            </div>
            <div class="box-content">
               ...
               <?php if(!empty($data['admin_capabilities'])){ ?>
                ...
                <input name="action_flag" id="action_flag" type="hidden" value="add">
                <!-- <div  class="col-md-6">
                    @if(!empty($data[0]['admin_capabilities']))
                    <div class="form-group">
                        <label class="col-sm-4 col-lg-3 control-label" for="admin_module" style="padding-bottom:10px"><b> Admin Module</b></label>                                                 
                    ...
                    <?php $i++; ?>
                        <div class="form-group">
                            <label class="col-sm-4 col-lg-3 control-label" for="admin_module"><input type="checkbox" name="admin_module[]" value="{{str_replace(" ","",$modules['slug'])}}|{{$modules['module']}}" onclick="Admincheck(this);" checked/>&nbsp;{{$modules['module']}}</label>
                            <div class="col-sm-6 col-lg-5 controls">
                                @foreach($modules['action'] as $activities)                               
                                    ...
                                    <label class="checkbox">
                                      <input type="checkbox" class="{{str_replace(" ","",'admin_'.$modules['module'])}}" name="{{str_replace(" ","",'admin_'.$modules['module'])}}[]" id="{{str_replace(" ","",'admin_'.$activities['name'])}}" value="{{$activities['name']}}|{{$activities['slug']}}|{{$activities['is_default']}}" <?php if($activities['is_default'] == true) { echo "checked"; } ?>/> {{$activities['name']}}
                                    </label>
                                @endforeach
                            </div>                                        
                        </div>
                    ...
                    <?php $i++; ?>
                        <div class="form-group">
                            <label class="col-sm-4 col-lg-3 control-label" for="portal_module"><input type="checkbox" name="portal_module[]" value="{{str_replace(" ","",$modules['slug'])}}|{{$modules['module']}}" onclick="Portalcheck(this);" checked/>&nbsp;{{$modules['module']}}</label>
                            <div class="col-sm-6 col-lg-5 controls">
                                @foreach($modules['action'] as $activities)                               
                                    <?php if($activities['is_default'] == true){ $portal_default[str_replace(" ","",$modules['module'])][]=str_replace(" ","",$activities['name']); } ?>
                                    <label class="checkbox">
                                      <input type="checkbox" class="{{str_replace(" ","",'portal_'.$modules['module'])}}" name="{{str_replace(" ","",'portal_'.$modules['module'])}}[]" id="{{str_replace(" ","",'portal_'.$activities['name'])}}" value="{{$activities['name']}}|{{$activities['slug']}}|{{$activities['is_default']}}" <?php if($activities['is_default'] == true) { echo "checked"; } ?>/> {{$activities['name']}}
                                    </label>
                                @endforeach
                            </div>                                        
                        </div>
                   ...
                   <div class="col-md-12">
                    ...
                    {
                      ?>
                      <label class="col-sm-2 col-md-2 col-lg-2 control-label cs-label" for="admin_module"><input type="checkbox" name="modules[]" onclick="Defaultcheck(this);" id='admin_{{$key}}' value="{{$key}}|{{$module}}" checked/>&nbsp;{{$module}}</label>
                                ...
                                <?php $i++; ?>
                                        ...
                                        <div class="col-sm-6 col-md-6 controls">
                                            @foreach($each['action'] as $activities)                               
                                                <?php if($activities['is_default'] == true){ $admin_default[$each['module']][]=$activities['name']; } ?>
                                                <label class="checkbox">
                                                  <input type="checkbox" class="{{str_replace(" ","",'admin_'.$each['module'])}}" name="{{str_replace(" ","",'admin_'.$each['module'])}}[]" id="{{'admin_'.$activities['name']}}" onclick="CheckParent('admin_{{$key}}','{{str_replace(" ","",$module)}}');" value="{{$activities['name']}}|{{$activities['slug']}}|{{$activities['is_default']}}" <?php if($activities['is_default'] == true) { echo "checked"; } ?>/><?php if($activities['is_default'] == true) { echo '<b>'.' '.$activities['name'].'</b>'; }else{echo ' '.$activities['name'];} ?> 
                                                </label>
                                            @endforeach
                                        </div>    
                             ...
                             if($each['module']==$module)
                             {
                                ...
                                <?php $i++; ?>
                                    <div class="col-sm-6 col-md-6 controls b-left-0">
                                        @foreach($each['action'] as $activities)                               
                                            <?php if($activities['is_default'] == true){ $portal_default[$each['module']][]=$activities['name']; } ?>
                                            <label class="checkbox">
                                              <input type="checkbox" class="{{str_replace(" ","",'portal_'.$each['module'])}}" name="{{str_replace(" ","",'portal_'.$each['module'])}}[]" id="{{'portal_'.$activities['name']}}" onclick="CheckParent('admin_{{$key}}','{{str_replace(" ","",$module)}}');" value="{{$activities['name']}}|{{$activities['slug']}}|{{$activities['is_default']}}" <?php if($activities['is_default'] == true) { echo "checked"; } ?>/> <?php if($activities['is_default'] == true) { echo '<b>'.$activities['name'].'</b>'; }else{echo $activities['name'];} ?> 
                                            </label>
                                        @endforeach
                                    </div>                                        
