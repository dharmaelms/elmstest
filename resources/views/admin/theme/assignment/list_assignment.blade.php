@section('content')
    ...
    <script>
        ...
        var $targetarr = [0, 2, 3, 4, 5, 6, 10];
    </script>
     <style type="text/css">
        .assignment-header th{
        ...
        <div class="col-md-12">
            <div class="box">
                ...
                @if (has_admin_permission(ModuleEnum::ASSIGNMENT, AssignmentPermission::ADD_ASSIGNMENT))
                    <div class="col-md-12 margin-bottom-20">
                        <div class="btn-toolbar pull-right clearfix margin-bottom-20">
                            <div class="btn-group">
                                <div class="btn-group">
                                    <a class="btn btn-primary btn-sm" href="{{URL::route('get-add-assignment')}}">
                                        <span class="btn btn-circle blue show-tooltip custom-btm">
                                            <i class="fa fa-plus"></i>
                                        </span>&nbsp;{{trans('admin/assignment.add_assignment')}}
                                    </a>&nbsp;&nbsp;
                                </div>
                            </div>
                        </div>
                ...
                @endif
                    ...
                    <table class="table table-advance" id="datatable-assignment">
                        <thead>
                            <tr class="assignment-header">
                                <th style="width: 5% !important;"><input type="checkbox" id="checkall" /></th>
                                <th style="width: 20% !important;">{{trans('admin/assignment.assignment_title')}}</th>
                                <th style="width: 5% !important;">{{trans('admin/assignment.submitted_users')}}</th>
                                <th style="width: 5% !important;">{{trans('admin/assignment.needs_grading')}}</th>
                                <th>{{trans('admin/assignment.posts')}}</th>
                                <th>{{trans('admin/assignment.users')}}</th>
                                <th>{{trans('admin/assignment.user_group')}}</th>
                                <th>{{trans('admin/assignment.start_date')}}</th>
                                <th>{{trans('admin/assignment.end_date')}}</th>
                                <th>{{trans('admin/assignment.cutoff_date')}}</th>
    ...
    <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static">
            <div class="modal-dialog">
                <div class="modal-content">
                                ...
                                <div class="box">
                                    <div class="box-title">
                                        ...
                                        <h3 class="modal-header-title">
                                            <i class="icon-file"></i>
                                            {{trans('admin/assignment.delete_assignment')}}
                                        </h3>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="modal-body" style="padding: 20px">
                        Are you sure you want to delete this {{strtolower(trans('admin/assignment.assignment'))}} ?
                    </div>
                    <div class="modal-footer">
                        <a class="btn btn-danger">{{trans('admin/assignment.yes')}}</a>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-footer" style="padding-right: 38px">
                        <a class="btn btn-success"><em class="fa fa-check-circle"></em>&nbsp;{{trans('admin/assignment.assign')}}</a>
                        <a class="btn btn-danger" data-dismiss="modal"><em class="fa fa-times-circle"></em>&nbsp;{{trans('admin/assignment.close')}}</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="modal fade" id="unassign-post" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static">
            <div class="modal-dialog">
                <div class="modal-content">
                            ...
                            <div class="col-md-12">
                                <div class="box">
                                    <div class="box-title">
                                        ...
                                        <h3 class="modal-header-title">
                                            <i class="icon-file"></i>
                                            {{trans('admin/assignment.unassign_post_madal')}}
                                        </h3>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="modal-footer" style="text-align: center !important">
                        <b>{{trans('admin/assignment.confir_unassign_post')}}</b>&nbsp;&nbsp;&nbsp;
                        <a class="btn btn-danger">{{trans('admin/assignment.yes')}}</a>
                        <a class="btn btn-success" data-dismiss="modal">{{trans('admin/assignment.close')}}</a>
                    </div>
                </div>
         ...
         <script type="text/javascript">
            var  order_by_var= "{{Input::get('order_by','7 desc')}}";
            var  order = order_by_var.split(' ')[0];
            var  _by   = order_by_var.split(' ')[1];

            function updateCheckBoxVals(){
                $allcheckBoxes = $('#datatable-assignment td input[type="checkbox"]');
                ...
                if($allcheckBoxes.length > 0)
                    if($allcheckBoxes.not(':checked').length > 0)
                        $('#datatable-assignment thead tr th:first input[type="checkbox"]').prop('checked',false);
                    else
                        $('#datatable-assignment thead tr th:first input[type="checkbox"]').prop('checked',true);
            }
            ...
            /* code for assignment DataTable begins here */
                var $datatable = $('#datatable-assignment');
                var datatableOBJ  = $('#datatable-assignment').on('processing.dt',function(event,settings,flag){
                    if(flag == true)
                        simpleloader.fadeIn();
                    else
                    ...
                    $('td:nth-child(2) div', '#datatable-assignment tr').each(function(){
                    if($(this).height() > 75){
                ...
                $('#datatable-assignment_filter input').unbind();
                    $('#datatable-assignment_filter input').bind('keyup', function(e) {
                        if(e.keyCode == 13) {
                ...
                $('#checkall').change(function(e){
                    $('#datatable-assignment td input[type="checkbox"]').prop('checked',$(this).prop('checked'));
                    $('#datatable-assignment td input[type="checkbox"]').trigger('change');
            ...
            $datatable.on('click','.assignment-assign',function(e){
                e.preventDefault();
                var $this = $(this);
                assignment_id = $this.data("key");
                ...
                $('.modal-footer .btn-success',$triggermodal).unbind('click').click(function(){
                    var $checkedboxes = $iframeobj.get(0).contentWindow.checkedBoxes;
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ URL::route('post-assign-assignment') }}",
                            data: 'ids='+$postdata+'&empty=true&feed='+feed+'&from='+action+'&key='+key,
                        })
            ...
            /* Code for loading the posts in the iframe for assignment starts here */
            ...
            $("select[name='feed']").change(function() {
                $('iframe').attr(
                    ...
                    "?input_type=radio&view=iframe&from=assignment&relid="+assignment_id
                );
            });  
            /* Code forloading the posts in the iframe for assignment ends here */

            $(document).on('click','.assignment-post-unassign',function(e){
                e.preventDefault();
                var $this = $(this);
                assignment_id = $this.data("key"); 
        ...
        $datatable.on('click','.assignment-post-assign',function(e){
            e.preventDefault();
            simpleloader.fadeIn();
            var $this = $(this);
            assignment_id = $this.data("key");
            ...
            $('.modal-footer .btn-success',$triggermodal).unbind('click').click(function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ URL::route('post-assign-assignment') }}",
                    data: 'ids='+$postdata+'&empty=true&feed='+feed+'&from='+action+'&key='+assignment_id,
                })
                .done(function( response ) {
                    if(response.flag == "success")
                        $('<div class="alert alert-success"><button class="close" data-dismiss="alert">×</button>'+response.message+'</div>').insertAfter($('.page-title'));
