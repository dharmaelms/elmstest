...
<div class="row custom-box">
    <div class="col-md-12">
          <div class="box">
             <!--  <div class="box-title">
                  <h3>Continue with the following actions:</i></h3>
              </div> -->  
          </div>
      </div>
    </div>
  <div class="row custom-box">
      <div class="col-md-4">
          <div class="box box-lightgreen">
              <div class="box-title">
                  <h3>{{ trans('admin/user.user_management_actions') }}</h3>
              </div>
          <div class="box-content">
            @if(has_admin_permission(ModuleEnum::CHANNEL, ChannelPermission::CHANNEL_ASSIGN_USER))
                @if($asset['status'] == 'ACTIVE')
                  <a href="{{URL::to('/cp/contentfeedmanagement?filter=ACTIVE&view=iframe&from=user&subtype=single&relid='.$uid)}}" class="btn btn-blue triggermodal" data-info="contentfeed">{{ trans('admin/user.assign_channel_to_user') }}</a>
                @else
                  <a class="btn btn-blue" title="{{ trans('admin/user.cant_assign_channel_to_inactive_user') }}">{{ trans('admin/user.assign_channel_to_user') }}</a>
                @endif
            @endif

            @if(has_admin_permission(ModuleEnum::USER_GROUP, UserGroupPermission::USER_GROUP_ASSIGN_USER))
                @if($asset['status'] == 'ACTIVE')
                    <a href="{{URL::to('/cp/usergroupmanagement/user-groups?filter=ACTIVE&view=iframe&from=user&relid='.$uid)}}" class="btn btn-blue triggermodal" data-info="usergroup">{{ trans('admin/user.assign_ug_to_user') }}
                    </a>
                @else
                    <a class="btn btn-blue" title="<?php echo trans('admin/user.cannot_assign_usergroup_to_inactive_user');?>">{{ trans('admin/user.assign_ug_to_user') }}</a>
                @endif
            @endif
          </div>
        </div>
  ...
  <div class="col-md-4">
      <div class="box box-lightgray">
          <div class="box-title">
                <h3 style="color:black"> {{ trans('admin/user.additional_actions') }}</h3>  
          </div>
          <div class="box-content">
            @if(has_admin_permission(ModuleEnum::USER, UserPermission::ADD_USER))
                <a class="btn btn-lightblue" href="{{ URL::to('cp/usergroupmanagement/add-user') }}">
                    {{ trans('admin/user.add_another_user') }}
                </a>
            @endif

            @if(has_admin_permission(ModuleEnum::USER, UserPermission::LIST_USER))
                <a  class="btn btn-lightblue" href="{{ URL::to('cp/usergroupmanagement') }}">
                    {{ trans('admin/user.view_all_users') }}
                </a>
            @endif
          </div>          
              </div>
    ...
    $('.triggermodal').click(function(e){
      ...
      $('.modal-footer .btn-success',$triggermodal).unbind('click').click(function(){
        ...
        if(!$.isEmptyObject($checkedboxes)){
          ...
          $.ajax({
          type: "POST",
          url: '{{URL::to('/cp/usergroupmanagement/assign-user/')}}/'+action+'/'+$key,
          data: 'ids='+$postdata
        })
