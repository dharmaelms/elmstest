@section('content')
	<div class="tabbable tabbable-tabdrop color-tabs">
		<ul class="nav nav-tabs center margin-btm-0">
			<li @if($filter == 'unattempted') class="active" @endif><a href="{{ URL::to('/assessment?filter=unattempted') }}">Unattempted ({{$count['unattempted']}})</a></li>
			<li @if($filter == 'attempted') class="active" @endif><a href="{{ URL::to('/assessment?filter=attempted') }}">Attempted ({{$count['attempted']}})</a></li>
		</ul>
			...
			<div class="tab-pane active">
				<div class="row">
					...
					<div class="facets-data col-lg-12 col-md-12 col-sm-12 col-xs-12 assessment-div">
						<div class="xs-margin"></div><!--space-->
						@include('portal.theme.default.assessment.quiz_ajax_load', ['quizzes' => $quizzes, 'attempt_detail' => $attempt_detail, 'filter' => $filter])
					</div><!--facets data div-->
