@section('content')
	<div class="tabbable tabbable-tabdrop color-tabs">
		<ul class="nav nav-tabs center margin-btm-0">
			<li @if($filter == 'unattempted') class="active" @endif><a href="{{ URL::to('/survey?filter=unattempted') }}"><i class="fa fa-times" aria-hidden="true"></i>{{Lang::get('survey.unattempted')}} ({{$count['unattempted']}})</a></li>
			<li @if($filter == 'attempted') class="active" @endif><a href="{{ URL::to('/survey?filter=attempted') }}"><i class="fa fa-check-square-o" aria-hidden="true"></i> {{Lang::get('survey.attempted')}} ({{$count['attempted']}})</a></li>
		</ul>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="facets-data col-lg-12 col-md-12 col-sm-12 col-xs-12 assessment-div">
							<div class="xs-margin"></div>
							@include('portal.theme.default.survey.survey_ajax_load', ['surveys' => $surveys, 'completed_list' => $completed_list, 'filter' => $filter])
						</div>
	...
	$(function() {
		...
		var survey_display_count = {{ $count[$filter] }};
		var start = 9;
		var stop = flag = true;
		$(window).scroll(function() {
			if(survey_display_count > 8 && stop) {
	        	if(($(window).scrollTop() + $(window).height()) > ($(document).height() - 100)) {
	        		if(flag) {
	        			flag = false;
		        		$.ajax({
		        			type: 'GET',
		        			url: "{{ url('survey?filter='.$filter) }}&start="+start
		        		}).done(function(e) {
		        			if(e.status == true) {
