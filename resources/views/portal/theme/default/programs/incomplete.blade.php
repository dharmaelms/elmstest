...
<div class="tabbable tabbable-tabdrop color-tabs">
	<ul class="nav nav-tabs center">
		...
		&nbsp; My {{trans('program.programs')}}</a></li>
		@endif
					...
					<div class="myactivity-section">
						<div id="parentHorizontalTab">
							<ul class="resp-tabs-list hor_1">
								<li> <a href="{{URL::to('program/what-to-watch')}}">
								{{trans('program.recent_posts')}}</a></li>
								<li class="resp-tab-item hor_1 resp-tab-active"><a href="{{URL::to('program/incomplete-posts')}}">{{trans('program.incomplete_post')}}</a></li>
							</ul>
							<div class="resp-tabs-container hor_1">
								...
								<div class="tab-content resp-tab-content hor_1 resp-tab-content-active">
									@if(count($packets) > 0)
									@include('portal.theme.default.programs.incomplete_ajax_load', ['packets' => $packets, 'favorites' => $favorites, 'program_details' => $program_details])
									@else
									<h4 align="center">{{trans('program.no_post_to_watch')}}</h4>
									@endif
								</div><!-- Incomplete Posts -->
							</div>
						</div>
					</div>
				</div><!--facets data div-->
			...
			if(count > 8 && stop) {
				if(($(window).scrollTop() + $(window).height()) > ($(document).height() - 100)) {
					if(flag) {
						...
						$.ajax({
							type: 'GET',
							url: "{{ url('program/incomplete-next-records?pageno=') }}"+pageno
						}).done(function(e) {
							if(e.status == true) {
