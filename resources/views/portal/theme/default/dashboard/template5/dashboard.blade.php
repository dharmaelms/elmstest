@section('content')
                    ...
                    <div id="latestcourses">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <div class="title-border"><h4 class="black font-weight-500">{{ Lang::get('dashboard.latest_content') }} &nbsp;&nbsp;<a href="#" class="font-12">{{ URL::to('program/what-to-watch') }}</a></h4>
                                ...
                                <div class="padding-lft-15">
                                    <table class="table" id="posts">
                                        <thead>
                                        <tr>
                                            <th width="200">{{ Lang::get('dashboard.name') }}</th>
                                            <th width="140">{{ Lang::get('dashboard.course_name') }}</th>
                                            <th width="60">{{ Lang::get('dashboard.content') }}</th>
                                            <!-- due to mongo load hidden status feature -->
                                        <!-- <th width="100" class="center">{{ Lang::get('dashboard.status') }}</th> -->
                                            <th width="80"  class="center">{{ Lang::get('dashboard.Q_&_A') }}</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
            ...
            <!-- END Latest Content -->
                @if($general->setting['assessments'] == "on")
                    <div id="assess">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <div class="title-border"><h4 class="black font-weight-500">{{ Lang::get('dashboard.assessements') }} &nbsp;&nbsp;<a href="#" class="font-12">{{ URL::to('assessment') }}</a></h4>
                                    <div class="nav-buttons pull-right">
                                ...
                                <div class="padding-lft-15">
                                    <table class="table" id="quiz">
                                        ...
                                        <tr>
                                            <th width="200">{{ Lang::get('dashboard.name') }}</th>
                                            <th width="100">{{ Lang::get('dashboard._of_questions') }}</th>
                                            <th width="80">{{ Lang::get('dashboard.dauration') }}</th>
                                            <th width="110">{{ Lang::get('dashboard.starts_on') }}</th>
                                            <th width="110">{{ Lang::get('dashboard.ends_on') }}</th>
                                        </tr>
                                        </thead>
                                        <tbody></tbody>
                                    </table>
        ...
        <div class="row" id="db-sidebar">
            <div class="col-md-4 col-sm-4 col-xs-12">
                <div class="annoucements-div1 sm-margin">
                    <div class="title-border"><h4 class="black font-weight-500">{{ Lang::get('dashboard.announcements') }}
            ...
            @if($general->setting['events'] == "on")
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <div class="calendar1 sm-margin">
                        <div class="title-border"><h4 class="black font-weight-500" style="line-height: 1.15;">{{ Lang::get('dashboard.calendar') }}</h4>
                        </div>
                ...
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <div class="event-container1 sm-margin">
                        <div class="title-border">
                            <h4 class="black font-weight-500">{{ Lang::get('dashboard.upcoming_events') }}
                                <span class="badge badge-danger" id="event-count">0</span>
                            </h4>
                            <div class="nav-buttons pull-right">
                                <div class="prev"><a href="" id="prevPageEvent"><i class="fa fa-angle-left"></i></a></div>
                                <div class="next"><a href="" id="nextPageEvent"><i class="fa fa-angle-right"></i></a></div>
                            </div>
                        </div>
                        <div class="blue-cs-border"><hr></div>
                        <ul class="padding-lft-30" id="events-list"></ul>
                        <p class="center"><a href="#" class="black font-12"><img src="{{URL::asset($theme.'/img/icons/more-icon.png')}}" alt="More" title="More">{{ Lang::get('dashboard.more') }}</a></p>
                    </div><!-- events -->
                </div>
            @endif
        </div>
    ...
    <script>
        ...
        $('#programs').on('click', '.view-details', function(e){
        ...
        var post = (function(){
            var limit = 3,
                ...
                content = '<tr data-id="{packet_id}" data-page-id="{page}">'+
                    '<td>{title} <a href="{url}" class="label label-primary"> View </a></td>'+
                    '<td data-program-name={feed_slug}></td>'+
                    '<td>{elements} items</td>'+
                    // due to mongo load hidden status feature
                        /*'<td class="status-badge" data-post-status="{packet_id}" align="center"></td>'+*/
                    '<td align="center" data-post-questions="{packet_id}"><span class="badge badge-danger">0</span></td>'+
                    '</tr>';
            function init(){
            ...
            function refresh() {
                $(position).find('tr').fadeOut('slow').delay(2000).remove();
                page = 1;
                lastPage = 1;
                next = true;
                nextPage();
        ...
        var quiz = (function(){
            var limit = 3,
                ...
                content = '<tr data-page-id="{page}">'+
                    '<td>{name} <a href="{url}" class="label label-primary">View</a></td>'+
                    '<td>{questions} Question(s)</td>'+
                    '<td>{duration}</td>'+
                    '<td>{start_date}</td>'+
                    '<td>{end_date}</td>'+
                    '</tr>';
                        ...
                        $.each(response.data, function(index,result) {
                            $(position).append(
                                content.replace(/{page}/g, page)
                                    ...
                                    .replace(/{questions}/g, result.questions.length+" ")
