        ...
        <div class="item white-bg content-row-dashboard10"> 
            <a href="{{url('/program/packets/'.$program['program_slug'])}}" title="{{$program['program_title']}}"> 
                <div class="packet packet-container-dashboard10"> 
                    ...
                    <?php
                    $category_name = $programs['category_details']->whereIn('category_id', $program['program_categories'])->first();
                    if (empty($category_name)) {
                        $category_name = trans("catalog/template_two.no_category");
                    } else {
                        $category_name = $category_name->category_name;
                    }
                    ?>
                    <div class="dashboard-catagory-layer" title="{{$category_name}}">{{str_limit($category_name, 20)}}</div>
                    @endif
                    <div> 
