...
class MigrateUserToUserGroupFeedRel extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update:userToUserGroupFeedRel  {from : User_id as Integer}
        ...
        foreach ($assigned_user_group_ids as $user_group_id) {
            try {
                ...
                foreach ($user_group_channel_ids as $channel_id) {
                    $userenrollmentdata = UserEnrollment::where('user_id', $user->uid)
                        ->where('entity_type', 'PROGRAM')
                        ->where('entity_id', $channel_id)
                        ->where('status', 'ENROLLED')
                        ->where('source_type', 'USER_GROUP')
                        ->where('source_id',  $user_group_id)
                        ->orderBy('enrolled_on', 'desc')
                        ->limit(1)
                        ->get()
                        ->toArray();
                        
                    if (empty($userenrollmentdata)) {
